...
computational level since the 1980s. Following this suggestion, \\
\textbf{WORDS!}\\
In one hand, image recognition approaches based on traditional machine learning algorithms are a 
two step process; where the first step is comprised upon the extraction of features from the data, 
and the last step lies in the training of a classifier. 
...
introduction of modifications such as \gls{nlb}\autocite{wang2018non} or Squeeze Excitation Blocks  
...
\gls{nlp} such as LSTMs and RNNs, particularly their struggles in capturing long range dependencies 
and efficient training. 

In a similar fashion to the impact AlexNet had on image recognition, transformers revolutionized 
the landscape of \gls{nlp} with their key component: \emph{Self-Attention}. This function assigns 
...
Another pivotal characteristic setting transformers apart from previous approaches on \gls{nlp} 
...
Continuing on with transformers within the \gls{nlp} domain, several additions 
...
analogously to a sequence of tokens in a \gls{nlp} application. \autoref{fig:rel_vit} provides an 
...
% Chat-GPT Paragraph, fact check and style adapt
...
Following the locality properties of \glspl{cnn} and globality of \glspl{vit}; it has been proven 
...
\cite{xiao2021early}, it refers to the presence of difficulties charactherizing the training 
